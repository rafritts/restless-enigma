...
public class BombeEndpoints {
    ...
    public @ResponseBody BombeBreakResponse breakEnigma(@RequestBody BombeBreakRequest request) {
        BombeResult result = bombeService.breakEnigma(request.getEncodedMessage(), request.getSearchPhrase());
        ...
        response.setBombeResultStatus(result.getBombeResultStatus());
        return response;
    }
